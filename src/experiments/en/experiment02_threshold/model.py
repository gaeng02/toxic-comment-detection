...
@dataclass
class GoodBadDistanceClassifier :
    ...
    def _predict_vector (self, vec : np.ndarray) -> int :
        ...
        if self.threshold_distance is not None :
            mask = (dists_all <= self.threshold_distance)
            
            if np.any(mask) :
                idx_candidates = np.where(mask)[0]

                if (self.k is not None) and (self.k > 0) and (len(idx_candidates) > self.k) :
                    idx_sorted = idx_candidates[np.argsort(dists_all[idx_candidates])]
                    idx_used   = idx_sorted[: self.k]
                else :
                ...
                if count_bad > count_good : return 1
                if count_good > count_bad : return 0
                
                return int(self.break_label)
