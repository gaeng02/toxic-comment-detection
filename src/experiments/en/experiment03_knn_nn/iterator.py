...
from src.experiments.en.experiment03_knn_nn.model import (
    build_knn_features,
    DistanceNN,
    DistanceNNConfig,
)
...
RESULTS_DIR = os.path.join("results", dataset, "experiment03_knn_nn")

result_num = 6
GOOD_DB_PATH = os.path.join("results", dataset, "experiment02_threshold", f"{result_num:02d}", "good_db_vectors.npy")
BAD_DB_PATH  = os.path.join("results", dataset, "experiment02_threshold", f"{result_num:02d}", "bad_db_vectors.npy")
...
def main() :
    ...
    for k in [1, 13, 25, 37, 49, 63, 75, 87, 99] : 
        print(f"[INFO] Running with k = {k} ...")
        
        result_path = os.path.join(RESULTS_DIR, f"{k:02d}")
        ...
        config_dict = {
            ...
            "good_db_size": int(len(good_db_vectors)),
            "bad_db_size": int(len(bad_db_vectors)),
        }
